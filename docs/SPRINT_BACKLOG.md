...
- [ ] Link a test user's Telegram account via the `/link <code>` flow — needs a public webhook URL/tunnel, see `docs/HANDOFF_LOG.md`.
- [ ] Trigger a `crm.lead` creation and confirm a Telegram message is delivered — blocked on the two items above; the enqueue→dispatch→log path is covered by a mocked adapter test in the meantime.
